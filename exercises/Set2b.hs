...
oddFactorial 1 = 1
oddFactorial n = if n `mod` 2 == 0 then oddFactorial (n-1)  else n * oddFactorial (n-1)
...
leftpad word n = if length word >= n then word else leftpad (" " ++ word) n 
...
countdown' current result = countdown' (current-1) (result ++ show current ++ "... ") 
